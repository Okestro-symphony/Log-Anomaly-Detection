...
def preprocessing_main():
    ...
    def get_unique_hosts(es):
        ...
        data = es.search(index= 'sym-metric-network*',body = body, track_total_hits= True, size=0)
    ...
    for host in hosts:
            ...
            "tmp_result" :{
              "date_histogram": {
                ...
                "fixed_interval": "5m"
                  ...
                  "aggs": {
                    "mean_network_in_bytes": {
                      "avg": {
                        "field": "system.network.in.bytes"
                  }
                },
                      "mean_network_out_bytes": {
                          "avg": {
        ...
        data['host_name'] = host
        data['mean_network_in_bytes'] = data['mean_network_in_bytes'].apply(lambda x: x['value'])
        data['mean_network_out_bytes'] = data['mean_network_out_bytes'].apply(lambda x: x['value'])
        if data.shape[0]<30:
            continue
    ...
    final_df.to_csv(data_dir+"/network.csv", index=False)
